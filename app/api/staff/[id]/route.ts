    ...
    const validCategoryIds: string[] = Array.isArray(categoryIds)
      ? (
          await prisma.bookingCategory.findMany({
            where: { id: { in: categoryIds }, salonId: check.salon.id },
            select: { id: true },
          })
        ).map((c) => c.id)
      : [];
...
// حذف پرسنل
// نکته: نوبت‌های قبلی به‌صورت اسنپ‌شات (staffName) ذخیره شده‌اند، پس حذف پرسنل تاریخچه را از بین نمی‌برد.
