 ...
 *   staffId?   — اختیاری: آیدی پرسنل خاص (اگه نباشه = بهترین)
 */
export async function GET(req: Request) {
  ...
  const staffIdParam = searchParams.get('staffId'); // null = بهترین
  ...
  const defaultDaySchedule = weeklySchedule[persianDayName];
  if (!defaultDaySchedule || !defaultDaySchedule.open) {
    // این روز سالن تعطیله
  ...
  // نتیجه نهایی: map از "HH:MM" → آرایه‌ای از پرسنل‌های آزاد در اون ساعت
  const slotStaffMap: Record<string, { id: string; name: string }[]> = {};

  for (const s of eligibleStaff) {
    // تعیین ساعت کاری این پرسنل در این روز
    ...
    // نوبت‌های ثبت‌شده برای این پرسنل (بر اساس staffName چون آرشیو قدیمی رشته‌ایه)
    // برای سیستم جدید از staffId استفاده می‌کنیم — فعلاً هر دو رو چک می‌کنیم
  ...
  // ── ۶. ساخت خروجی نهایی ─────────────────────────────────────────────────
  // هر اسلات: ساعت + لیست پرسنل‌های آزاد
  const slots = Object.entries(slotStaffMap)
    .map(([time, availableStaff]) => ({ time, availableStaff }))
    .sort((a, b) => a.time.localeCompare(b.time));

  // لیست تمام پرسنل واجد شرایط (برای نمایش گزینه انتخاب پرسنل)
