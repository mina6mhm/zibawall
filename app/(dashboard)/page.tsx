      ...
      <div className="flex flex-col min-h-screen bg-white pb-24">
        ...
        <div className="px-4 mt-4 md:mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {isLoading ? (
              // اسکلتون‌های لودینگ (ارتفاع کارت جدید و کوچک‌تر)
              [1, 2, 3, 4].map((n) => (
              ...
              filteredSalons.map((salon) => {
                ...
                return (
                  // --- شروع تغییرات اصلی برای ظاهر کارت (افقی، جمع‌وجور، مطابق طرح) ---
                  <div 
                    key={salon.id}
                      ...
                      <div className="flex items-start justify-between gap-1">
                        <div className="flex items-center gap-1 min-w-0">
                          <button 
                            ...
                            className={`shrink-0 transition-colors ${isCurrentSalonBookmarked(salon.id) ? 'text-[#824c71]' : 'text-zinc-400'}`}
                          >
                            <BookmarkIcon isActive={isCurrentSalonBookmarked(salon.id)} className="w-3.5 h-3.5" />
                          </button>
                  ...
                  // --- پایان تغییرات اصلی برای ظاهر کارت ---
                );
              })
            ) : (
