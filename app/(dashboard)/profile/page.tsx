...
export default function ProfilePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white pb-24">
      ...
      <div className="bg-white px-4 pt-6 pb-4">
        <div className="max-w-lg mx-auto">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-[#824c71] rounded-2xl flex items-center justify-center text-white shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                {userData.username && (
                  <span className="text-[#824c71] text-xs font-medium flex items-center gap-1">
                    <AtSign className="w-3 h-3" />{userData.username}
                  </span>
                )}
                {userData.phone && (
                  <span className="text-[#824c71]/60 text-xs flex items-center gap-1" dir="ltr">
            ...
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-xl bg-[#e3c9dc]/25 text-[#824c71] hover:bg-[#e3c9dc]/45 transition-colors shrink-0"
            >
      ...
      {/* تب‌ها */}
      <div className="bg-white border-b border-[#e3c9dc]/40 px-4 mt-0.5">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-medium border-b-2 transition-colors ${
                activeTab === key
                  ...
                  : 'border-transparent text-[#824c71]/40'
              }`}
            >
      ...
      <div className="max-w-lg mx-auto w-full px-4 mt-4">
        ...
        {/* تب اطلاعات */}
        {activeTab === 'info' && (
          <div className="bg-white border border-[#e3c9dc]/40 rounded-2xl p-4 space-y-4">
            <div>
              <label className="block text-xs font-medium text-[#824c71] mb-1.5">شماره موبایل</label>
              <input
                value={userData.phone}
                disabled
                dir="ltr"
                className="w-full border border-[#e3c9dc]/50 bg-[#e3c9dc]/10 rounded-xl px-3.5 py-2.5 text-sm text-[#824c71]/60 cursor-not-allowed text-left"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#824c71] mb-1.5">نام و نام خانوادگی</label>
              <input
                value={userData.name}
                onChange={(e) => setUserData({...userData, name: e.target.value})}
                className="w-full border border-[#e3c9dc]/60 rounded-xl px-3.5 py-2.5 text-sm focus:border-[#824c71] focus:ring-2 focus:ring-[#e3c9dc]/40 outline-none transition-all"
                placeholder="نام خود را وارد کنید"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#824c71] mb-1.5">نام کاربری</label>
              <div className="relative">
                <AtSign className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#824c71]/50" />
                <input
                  value={userData.username}
                  onChange={(e) => setUserData({...userData, username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '')})}
                  placeholder="username"
                  dir="ltr"
                  className="w-full border border-[#e3c9dc]/60 rounded-xl px-3.5 py-2.5 pl-10 text-sm text-left focus:border-[#824c71] focus:ring-2 focus:ring-[#e3c9dc]/40 outline-none transition-all"
                />
              </div>
            </div>
            <button
        ...
        {/* تب کسب‌وکار */}
        {activeTab === 'business' && (
          <div className="bg-white border border-[#e3c9dc]/40 rounded-2xl p-4">
            {salonData ? (
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-[#e3c9dc]/15 rounded-xl">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-[#e3c9dc] shrink-0">
                    <Store className="w-5 h-5 text-[#824c71]" strokeWidth={1.5} />
                  </div>
                  <div>
                    <h3 className="font-bold text-zinc-900 text-sm">{salonData.name}</h3>
                    <p className="text-xs text-[#824c71]/70 mt-0.5">{salonData.province}، {salonData.city}</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Link
                    ...
                    className="flex flex-col items-center gap-1.5 bg-[#e3c9dc]/25 text-[#824c71] rounded-xl py-3 text-xs font-medium hover:bg-[#e3c9dc]/45 transition-colors"
                  >
              ...
              <div className="text-center py-8">
                <div className="w-16 h-16 bg-[#824c71] rounded-2xl flex items-center justify-center mx-auto mb-4">
                ...
                <p className="text-sm text-[#824c71]/70 mb-6 leading-relaxed">کسب‌وکار خود را رایگان ثبت کنید</p>
                <Link
                ...
                >
                  <Sparkles className="w-4 h-4" /> شروع ثبت‌نام
                </Link>
              </div>
            )}
          </div>
