          ...
          <div className="space-y-2 md:space-y-8 animate-fade-in">
            ...
            <section className="flex flex-col md:space-y-6 mt-4 md:mt-8">
              ...
              <div className="flex flex-col md:space-y-3">
                ...
                {/* بخش انتخاب و نمایش نقشه منطبق با صفحه ویرایش */}
        ...
        {step === 2 && (
          <div className="space-y-2 md:space-y-8 animate-fade-in">
            <section className="flex flex-col md:space-y-6">
              ...
              {/* هدر بخش خدمات */}
              <div className="flex flex-row items-center justify-between border-b-2 border-zinc-50 md:border-zinc-100 pb-3 pt-2 md:pt-0">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="text-zinc-800 w-5 h-5 md:w-6 md:h-6" />
                  <h2 className="text-[16px] md:text-lg font-bold text-zinc-900">خدمات قابل ارائه</h2>
                </div>
                <span className="text-[11px] md:text-xs text-rose-600 bg-rose-50 px-2 py-0.5 md:px-3 md:py-1 rounded-full font-medium">
                  {selectedTags.length.toLocaleString('fa-IR')} خدمت انتخاب شده
                </span>
              </div>

              <p className="text-[13px] md:text-sm text-zinc-500 mt-2 md:mt-0 px-1 md:px-0">
                جزئیات خدماتی که در سالن شما ارائه می‌شود را با دقت انتخاب کنید تا مشتریان راحت‌تر شما را پیدا کنند.
              </p>
              
              {/* لیست آکاردئونی خدمات */}
              <div className="flex flex-col md:space-y-4 mt-2 md:mt-0">
                {allCategories.map((category) => {
                  ...
                  return (
                    <div key={category} className="border-b border-zinc-100 md:border-b-0 md:border md:border-zinc-200 md:rounded-xl md:overflow-hidden md:bg-white transition-all">
                      <button 
                        type="button" 
                        onClick={() => toggleCategory(category)} 
                        className="w-full flex items-center justify-between px-1 py-4 md:p-4 bg-transparent md:bg-white md:hover:bg-zinc-50 transition-colors focus:outline-none"
                      >
                        <div className="flex items-center gap-2 md:gap-3">
                          <span className="text-[14px] md:text-base font-medium text-zinc-800">{category}</span>
                          {selectedCount > 0 && (
                      ...
                      {isExpanded && (
                        <div className="px-1 pb-4 pt-1 md:p-4 md:border-t md:border-zinc-100 flex flex-col gap-4 animate-fade-in">
                          {/* لیست تگ‌ها */}
                          <div className="flex flex-wrap gap-2">
                              ...
                              if (isDefault) {
                                return (
                                  <button 
                                    ...
                                    className={`px-4 py-2 rounded-full md:rounded-xl text-[13px] md:text-sm font-medium transition-all ${
                                      isSelected
                                        ? 'bg-rose-50 text-rose-700 md:border md:border-rose-200'
                                        : 'bg-zinc-50 text-zinc-600 hover:bg-zinc-100 md:bg-white md:border md:border-zinc-200 md:hover:bg-zinc-50'
                                    }`}
                                  >
                                    {service}
                                  </button>
                                ...
                                return (
                                  <div 
                                    key={service} 
                                    className={`flex items-center gap-1 pl-2 pr-3 py-1.5 md:py-1.5 rounded-full md:rounded-xl text-[13px] md:text-sm font-medium transition-all ${
                                      isSelected 
                                        ? 'bg-amber-50 text-amber-700 md:border md:border-amber-300' 
                                        : 'bg-zinc-50 text-zinc-600 md:bg-white md:border md:border-dashed md:border-amber-300 md:text-amber-600'
                                    }`}
                                  >
                                    <button 
                                      type="button" 
                                    ...
                                    <button 
                                      ...
                                      className={`p-1 rounded-full transition-colors ${
                                        isSelected ? 'text-amber-500 hover:bg-amber-200' : 'text-zinc-400 hover:bg-zinc-200 md:text-amber-400 md:hover:bg-amber-100'
                                      }`}
                                      title="حذف کامل این خدمت"
                                    >
                                      <X className="w-4 h-4 md:w-4 md:h-4" strokeWidth={2.5} />
                                    </button>
                                  </div>
                                );
                              }
                          ...
                          <div className="flex items-center gap-2 mt-2 pt-3 border-t border-zinc-100/80 md:border-zinc-100">
                            <input 
                              type="text" 
                              ...
                              placeholder={`افزودن خدمت به ${category}...`} 
                              className="flex-1 text-[13px] md:text-sm px-0 py-2 md:px-4 md:py-2.5 bg-transparent md:bg-white rounded-none md:rounded-xl border-0 md:border md:border-zinc-200 focus:outline-none focus:md:border-zinc-300 focus:md:ring-1 focus:md:ring-zinc-300 transition-all placeholder:text-zinc-400"
                            />
                            <button 
                              type="button" 
                              onClick={() => handleAddCustomTag(category)}
                              className="px-3 py-2 md:px-4 md:py-2.5 bg-zinc-100 text-zinc-700 hover:bg-zinc-200 rounded-lg md:rounded-xl transition-colors flex items-center justify-center gap-1 font-medium text-[13px] md:text-sm whitespace-nowrap"
                            >
                              <Plus className="w-4 h-4" /> <span className="hidden md:inline">افزودن</span>
                            </button>
                          </div>
                        </div>
                      )}
